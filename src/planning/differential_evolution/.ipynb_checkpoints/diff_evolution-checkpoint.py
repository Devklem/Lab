...
class DE_algo:
    ...
    def obj(self, angles, *state):
        ...
        # objective function
        OFV = interpn(self.points, self.cube_3d, state_new, method='nearest') * 10 / length_constraint - penalty
    ...
    def DE_step(self, OFV=0, azi_l=None, incl_l=None, traj=None, init_pos=None, pop_size=100, maxiter=1000, F=0.7,
                bounds=[(0, 180), (0, 180), (0, 180), (0, 92), (0, 92), (0, 92)],
                length=10, angle_constraint=10, strategy='best1bin'
                ):
        """
        ...
        if init_pos != None:
            azi_l = [0, 0]
            incl_l = [0, 0]
            state = ([init_pos[0], init_pos[1], init_pos[2]], incl_l, azi_l, angle_constraint, length)
            traj_x = [init_pos[0]]
            traj_y = [init_pos[1]]
            traj_z = [init_pos[2]]
        else:
            ...
            traj_y = list(traj[1])
            traj_z = list(traj[2])
            state = ([traj[0][-1], traj[1][-1], traj[2][-1]], incl_l, azi_l, angle_constraint, length)

        de_sol = differential_evolution(self.obj2, bounds, args=(state), mutation=F,
                                        popsize=pop_size, strategy=strategy,
                                        maxiter=maxiter, updating='deferred', disp=False).x
        incl_l.append(de_sol[0])
        azi_l.append(de_sol[1])
        step = self.get_vec(incl_l[-1], azi_l[-1], length=length)

        # if len(azi_l) % 10 == 0:
        #     plot_results(self.old_cube, traj_x, traj_y, traj_z)
        ...
        return OFV, np.stack([traj_x, traj_y, traj_z]), azi_l, incl_l

    def DE_planning(self, pop_size=100, maxiter=1000, F=0.7, cr=0.7, strategy='best1bin',
                    bounds=[(0, 180), (0, 180), (0, 180), (0, 92), (0, 92), (0, 92)],
        ...
        dic = {'Step': [],
               ...
               'Y': [],
               'Z': []}
        while (traj_z[-1] <= self.old_cube.shape[2] - 2.1 * length) and (
                traj_y[-1] <= self.old_cube.shape[1] - 2.1 * length) and \
                (traj_x[-1] <= self.old_cube.shape[0] - 2.1 * length):
            de_sol = differential_evolution(self.obj2, bounds, strategy=strategy, args=(state), mutation=F,
                                            popsize=pop_size, \
                                            maxiter=maxiter, updating='deferred', disp=False).x
            incl_l.append(de_sol[0])
            azi_l.append(de_sol[1])
        ...
        print(OFV)
        df = pd.DataFrame(dic)
        return OFV, np.stack([traj_x, traj_y, traj_z]), df, incl_l, azi_l
...
def plot_results_2(volume_cube, traj_corr, traj_plan=None, x_coords=2000, y_coords=500, z_coords=500):
    """
    ...
    fig = px.imshow(volume_cube[int(traj_x[-1]), :, :], color_continuous_scale='rainbow', origin='lower', aspect='auto',
                    labels={f'x': 'X, m', 'y': 'Y, m ', 'color': 'Oil saturation, frac'},
                    x=z,
                    y=y,
                    )

    fig.update_xaxes(showticklabels=True)
    fig.add_scatter(x=z_t, y=y_t, name='Corrected', marker_color="black",
    ...
    fig.add_scatter(x=z_t, y=y_t, name='Corrected', line=dict(width=4),
                    marker_color="black", mode='lines', showlegend=True)

    fig2_1 = px.imshow(volume_cube[:, :, int(traj_z[-1])], color_continuous_scale='rainbow', origin='lower',
                       aspect='auto',
    ...
    fig2_1.add_scatter(x=[x_t[-1]], y=[y_t[-1]], name='Corrected', line=dict(width=300),
                       marker_color="black", mode='markers',
                       showlegend=True)

    if traj_plan is not None:
        fig2_1.add_scatter(x=[traj_x_plan[-1]], y=[traj_y_plan[-1]], name='Planned', line=dict(width=300),
                           marker_color="#8c564b", mode='markers',
                           showlegend=True)

    if traj_z[-1] + step > volume_cube.shape[2]:
        step = 0
    fig2_2 = px.imshow(volume_cube[:, :, int(traj_z[-1] + step)], color_continuous_scale='rainbow', origin='lower',
                       aspect='auto',
                       labels={f'x': 'Y, m', 'y': 'TVD, m ', 'color': 'Oil saturation, frac'},
                       x=x,
                       y=y)
    ...
    fig3 = px.imshow(volume_cube[:, int(traj_y[-1]), :], color_continuous_scale='rainbow', origin='lower',
                     aspect='auto',
                     labels={f'x': 'X, m', 'y': 'TVD, m ', 'color': 'Oil saturation, frac'},
                     x=z,
                     y=x,
                     )
    ...
    fig3.add_scatter(x=z_t, y=x_t, name='Corrected',
                     marker_color="black", line=dict(width=4), mode='lines+text',
                     text=[f'{i}' for i in range(len(traj_corr[0]))],
                     showlegend=True)
